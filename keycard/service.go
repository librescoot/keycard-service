...
type Service struct {
	...
	artificialDepartureExpected bool      // true when the next departure is from our own StartDiscovery restart
...
func (s *Service) handleTagEvent(event hal.TagEvent) {
	...
	case hal.TagArrival:
		...
		if s.handleTagDetection(uid) {
			...
			s.artificialDepartureExpected = true
			if err := s.nfc.StartDiscovery(100); err != nil {
...
func (s *Service) handleTagDetection(uid string) bool {
...
func (s *Service) handleTagDeparture() {
	...
	if s.artificialDepartureExpected {
		s.artificialDepartureExpected = false
		s.logger.Debug("Restart-triggered departure, debounce maintained", "lastTappedUID", s.lastTappedUID)
	} else {
